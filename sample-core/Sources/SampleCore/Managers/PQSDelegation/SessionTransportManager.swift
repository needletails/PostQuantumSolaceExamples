...
public actor SessionTransportManager: SessionTransport {
    ...
    public init(logger: NeedleTailLogger) {
        self.logger = logger
    }
    
    // MARK: - Public Methods
    
    public func setConnection(_ connection: IRCConnection) async {
    ...
    public func sendMessage(
        ...
        guard let recipientNick = NeedleTailNick(name: metadata.secretName, deviceId: metadata.deviceId) else {
            logger.log(level: .error, message: "Failed to create nickname for recipient: \(metadata.secretName)")
            throw SessionTransportError.invalidNickname
        }
        try await connection.transportMessage(
            command: IRCCommand.privMsg([.nick(recipientNick)], encodedString)
        )
    ...
    public func publishUserConfiguration(_ configuration: SessionModels.UserConfiguration, recipient secretName: String, recipient identity: UUID) async throws {
        logger.log(level: .info, message: "Publishing user configuration to recipient: \(secretName)")
        
        guard let connection = connection else {
            logger.log(level: .error, message: "No IRC connection available for publishing user configuration")
            throw SessionTransportError.noConnection
        }
        ...
        guard let recipientNick = NeedleTailNick(name: secretName, deviceId: identity) else {
            logger.log(level: .error, message: "Failed to create nickname for recipient: \(secretName)")
            throw SessionTransportError.invalidNickname
        }
        ...
        logger.log(level: .info, message: "User configuration published successfully to \(secretName)")
    }
    
    public func findConfiguration(for secretName: String) async throws -> SessionModels.UserConfiguration {
    ...
    public func fetchOneTimeKeys(for secretName: String, deviceId: String) async throws -> SessionModels.OneTimeKeys {
    ...
    public func fetchOneTimeKeyIdentities(for secretName: String, deviceId: String, type: SessionModels.KeysType) async throws -> [UUID] {
    ...
    public func updateOneTimeKeys(for secretName: String, deviceId: String, keys: [SessionModels.UserConfiguration.SignedOneTimePublicKey]) async throws {
    ...
    public func updateOneTimeMLKEMKeys(for secretName: String, deviceId: String, keys: [SessionModels.UserConfiguration.SignedMLKEMOneTimeKey]) async throws {
    ...
    public func batchDeleteOneTimeKeys(for secretName: String, with id: String, type: SessionModels.KeysType) async throws {
    ...
    public func deleteOneTimeKeys(for secretName: String, with id: String, type: SessionModels.KeysType) async throws {
    ...
    public func publishRotatedKeys(for secretName: String, deviceId: String, rotated keys: SessionModels.RotatedPublicKeys) async throws {
    ...
    public func createUploadPacket(secretName: String, deviceId: UUID, recipient: SessionModels.MessageRecipient, metadata: Data) async throws {
    ...
    public enum SessionTransportError: Error {
        case noConnection
