//
//  WebSocketTransportManager.swift
...
/**
 _WebSocketTransportManager_ conforms to **SessionTransport**. This protocol is how **PostQuantumSolace** makes the required Network Calls, it is up to the consumer to implement transport for PQSSession to function properly. Please read the **SessionTransport** documnetation in order to understand how to use each protocol method.
 **/

public actor WebSocketTransportManager: SessionTransport {
    
    // MARK: - Properties
    public let logger: NeedleTailLogger
    public let socket: WebSocketClient
    public let taskLoop = NTKLoop()
    // MARK: - Initialization
    
    public init(logger: NeedleTailLogger, socket: WebSocketClient) {
        self.logger = logger
        self.socket = socket
    }
    ...
    public func sendMessage(
        ...
        guard let sessionContext = await PQSSession.shared.sessionContext else {
            return
        }
        let myDeviceIdentity = sessionContext.sessionUser.deviceId
        ...
        let packet = MessagePacket(
            id: UUID().uuidString,
            flag: .privateMessage,
            senderSecretName: mySecretName,
            sender: myDeviceIdentity,
            recipientSecretName: metadata.secretName,
