...
public final class PQSSessionDelegateWrapper: PQSSessionDelegate, @unchecked Sendable {
    ...
    // MARK: - Properties
    public unowned let messageReciever: MessageReceiverManager
    public let pqsSession: PQSSession
    ...
    public func processMessage(
        ...
        do {
            // Check if this is a persist message first
            if let decoded = try? BinaryDecoder().decode([String: Bool].self, from: message.metadata),
               decoded.values.first == true {
                logger.log(level: .debug, message: "Message is marked for persistence, returning true")
                return true // If we are persisting messages return true
            }
            
            // Check for transportInfo to determine message type
            guard let transportData = message.transportInfo else {
                logger.log(level: .debug, message: "Message from \(senderSecretName) has no transportInfo, cannot determine message type")
                return false
            }
            
            let transportMetadata = try BinaryDecoder().decode(TransportMetadata.self, from: transportData)
            logger.log(level: .debug, message: "Message from \(senderSecretName) has messageFlag: \(transportMetadata.messageFlag)")
            guard let cache = await pqsSession.cache else { return false }
            switch transportMetadata.messageFlag {
            case .friendshipStateRequest:
                self.logger.log(level: .info, message: "Received friendship state request from: \(senderSecretName)")
                ...
                let contactSecretName = isMe ? message.recipient.recipientDescription : senderSecretName
                self.logger.log(level: .info, message: "Creating/updating contact for: \(contactSecretName) with friendship state: \(friendshipMetadata.theirState)")
                do {
                    let contact = try await pqsSession.createContact(
                        secretName: contactSecretName,
                        friendshipMetadata: friendshipMetadata,
                        requestFriendship: false)
                    self.logger.log(level: .info, message: "Successfully created/updated contact object: \(contact.id) for \(contactSecretName)")
                    // Note: createContact should trigger the createdContact or updateContact callback
                    // in MessageReceiverManager which will add/update the contact in the contacts array
                } catch {
                    self.logger.log(level: .error, message: "Failed to create/update contact for \(contactSecretName): \(error)")
                    throw error
                }
            ...
            case .communicationSynchronization:
                ...
                case .channel(let channelName):
                    // Channel synchronization (group communication)
                    do {
                        // Find existing channel communication
                        communicationModel = try await pqsSession.findCommunicationType(
                            cache: cache,
                        ...
                        let decoded = try BinaryDecoder().decode(ChannelInfo.self, from: message.metadata)
                        
                        try await pqsSession.createCommunicationChannel(
                ...
                case .personalMessage:
                    // Personal messages don't need synchronization - they are created automatically
                    // with sharedId when messages are sent via writeTextMessage
                    self.logger.log(level: .debug, message: "Personal message synchronization received - not needed, communication created automatically")
                    return false
                    
                default:
                    // Handle any other recipient types gracefully
                    self.logger.log(level: .warning, message: "Unsupported recipient type for synchronization: \(message.recipient)")
                    return false
                }
            ...
            case .contactCreated:
                guard let mySecretName = await pqsSession.sessionContext?.sessionUser.secretName else { 
                    self.logger.log(level: .error, message: "Cannot process contactCreated message: session context not available")
                    return false 
                }
                //This can happen on multidevice support when a sender is also sending a message to it's master/child device.
                let isMe = senderSecretName == mySecretName
                self.logger.log(level: .info, message: "Received Contact Created message from: \(senderSecretName) (isMe: \(isMe))")

                // Create a contact for the sender on the recipient's side
                // This ensures both parties have each other as contacts
                if !isMe {
                    let contactSecretName = senderSecretName
                    
                    // Always try to create/get the contact
                    // If it already exists, createContact should return it and trigger updateContact callback
                    // which will ensure it's in the UI list
                    self.logger.log(level: .info, message: "Creating/getting contact for sender: \(contactSecretName)")
                    do {
                        let contact = try await pqsSession.createContact(
                            secretName: contactSecretName,
                            requestFriendship: false
                        )
                        self.logger.log(level: .info, message: "Successfully created/retrieved contact object: \(contact.id) for \(contactSecretName)")
                        // Note: createContact with requestFriendship: false will:
                        // - If new: trigger createdContact callback and synchronize callback (which sends acknowledgment)
                        // - If existing: trigger updateContact callback (which ensures it's in UI list)
                        // Both callbacks will add/update the contact in the UI via MessageReceiverManager
                    } catch {
                        self.logger.log(level: .error, message: "Failed to create/get contact for \(contactSecretName): \(error)")
                        // Even if createContact fails, we should check if contact is in UI and add it if needed
                        // But we can't easily get Contact object without createContact, so we'll rely on
                        // the contact being added when it's next accessed or when contacts are reloaded
                        self.logger.log(level: .info, message: "Continuing despite contact creation error (contact may already exist)")
                    }
                }
            case .addContacts:
            ...
            default:
                logger.log(level: .debug, message: "Message from \(senderSecretName) has unhandled messageFlag: \(transportMetadata.messageFlag), treating as passthrough")
