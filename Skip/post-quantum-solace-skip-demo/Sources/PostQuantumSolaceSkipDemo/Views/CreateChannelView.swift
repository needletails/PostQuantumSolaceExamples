        ...
        Task {
            do {
                ...
                guard let admin = await session.pqsSession.sessionContext?.sessionUser.secretName else {
                    logger.log(level: .error, message: "Cannot create channel: session context not available")
                    await MainActor.run {
                        isCreating = false
                        showCreateChannelFailed = true
                    }
                ...
                guard let channel = NeedleTailChannel(finalChannelName) else {
                    logger.log(level: .error, message: "Invalid channel name: \(finalChannelName)")
                    await MainActor.run {
                        isCreating = false
                        showCreateChannelFailed = true
                    }
                ...
                logger.log(level: .error, message: "Failed to create channel: \(error)")
                await MainActor.run {
                    isCreating = false
                }
            }
